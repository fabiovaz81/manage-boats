...
export class ChatPanelComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.refreshChat();
        //const getNewMessagesTimer = timer(5000, 2500);
        //getNewMessagesTimer.subscribe(this.getNewMessages);     
    ...
    refreshContacts(): void {
        ...
        {
            console.log("Falha ao sincronizar contatos: ", exc);
            setTimeout(() => {
                this.refreshContacts();
            }, 1000);
        }        
    ...
    refreshChat(): void {        
        ...
        try {
            //console.log("Atualizando chat...", this.selectedContact, this.chat);

            if (this.selectedContact && this.chat && this.chat.lastMessageId) {

                //console.log("oie:", this.chat);

                // Load the chat
                this._chatPanelService.getNewMessages(this.selectedContact.id, this.chat.lastMessageId).then((resultado) => {

                    if (resultado && resultado.lastMessageId) {

                        this.chat.lastMessageId = resultado.lastMessageId;
                        //this.chat.dialog = resultado.dialog;
                        //console.log("Tem novas mensagens, xd", this.chat, resultado.dialog);

                        resultado.dialog.forEach(mensagem => {
                            this.chat.dialog.push(mensagem);
                        });                        
            ...
            console.log("Falha ao sincronizar chat: ", exc);
    ...
    private _prepareChatForReplies(): void {
        ...
        setTimeout(() => {
            ...
            //this._replyInput.nativeElement.reset();
    ...
    reply(event): void {
        ...
        // Update the server
        this._chatPanelService.updateChat(this.chat.contactId, this.chat.dialog).then(response => {
